...
export async function loadAndDisplayResources(courseId, resourceType, section) {
  const resources = await loadResourcesForCourse(
    courseId,
    resourceType,
    section
  );
...
export function renderResourcePanel(
    ...
    panel.innerHTML = `
      <div class="resource-section-header" data-resource-type="${resourceType}">
        <div class="resource-section-title">
          ...
          <span class="resource-type-label">${getResourceTypeLabel(
            resourceType
          )}</span>
  ...
  const resourcesHTML = `
    <div class="resource-section-header" data-resource-type="${resourceType}">
      <div class="resource-section-title">
        ...
        <span class="resource-count">(${resources.length} resource${
      ...
      ${resources
        ...
        <div class="note-item">
          <div class="note-info">
            ...
            ${
              resource.section
                ? `<span class="section-badge">Section ${resource.section}</span>`
                : ""
            }
          </div>
          <a href="${
            resource.url
          }" target="_blank" class="note-link" rel="noopener noreferrer">
            📎 View Resource
          </a>
        </div>
      `
  ...
  if (header) {
    header.addEventListener("click", () =>
      toggleResourceSection(resourceType, panel)
    );
  }
}
...
export function clearAllResourcePanels() {
  ...
  panels.forEach((panelId) => {
    ...
    if (panel) {
      panel.innerHTML = `
        <div class="resource-section-header" data-resource-type="${panelId.replace(
          "course-",
          ""
        )}">
          <div class="resource-section-title">
...
export function switchResourceTab(tabName) {
  ...
  document
    .querySelector(`[data-resource="${tabName}"]`)
  ...
  // If switching to drive files tab, initialize the file manager
  if (tabName === "drive-files") {
    initDriveFileManager();
    return;
  }
    ...
    setTimeout(() => {
      ...
      if (panel && panel.querySelector(".resource-section-content")) {
        ...
        if (
          ...
          content.querySelector(".note-item")
        ) {
...
export function openRoutinesModal() {
  ...
  // Initialize the drive file manager first (default tab)
  setTimeout(() => {
    initDriveFileManager();
  }, 100);

  // Load courses data for other tabs
...
export function actuallyCloseRoutinesModal() {
  ...
  resetDriveFileManager(); // Reset drive file manager state
}

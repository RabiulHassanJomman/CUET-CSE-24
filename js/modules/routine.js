...
export async function loadExtraClassesFromFirebase() {
  try {
    ...
    const snapshot = await db
      ...
      .get();
    extraClassesArray = snapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
  } catch (err) {
...
export function renderExtraClasses() {
  ...
  if (!extraClassesArray.length) {
    extraContainer.innerHTML =
      '<h3>📚 Extra Classes</h3><div class="no-extra-classes">No extra classes scheduled yet.</div>';
    return;
  }

  const extraClassesHTML = extraClassesArray
    .map((extraClass) => {
      const date = extraClass.date
        ? new Date(
            extraClass.date.seconds
              ? extraClass.date.seconds * 1000
              : extraClass.date
          ).toLocaleDateString()
        : "";
      const time = extraClass.time || "";
      const dayName = extraClass.day
        ? extraClass.day.charAt(0).toUpperCase() + extraClass.day.slice(1)
        : "";
      const subsection = extraClass.subsection
        ? ` | Sub: ${extraClass.subsection}`
        : "";
...
export function switchRoutineFilter(subsection) {
  ...
  document
    .querySelector(`[data-subsection="${subsection}"]`)
...
async function deleteExpiredItems() {
  ...
  try {
    ...
    const expiredExtraClasses = extraClassesSnapshot.docs.filter((doc) => {
      ...
      const extraClassDate = new Date(
        extraClassData.date.seconds
          ? extraClassData.date.seconds * 1000
          : extraClassData.date
      );
    ...
    if (expiredExtraClasses.length > 0) {
      console.log(
        `Auto-deleted ${expiredExtraClasses.length} expired extra classes`
      );
    }
  } catch (error) {

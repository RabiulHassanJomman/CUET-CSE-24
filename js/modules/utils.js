...
export async function deleteExpiredItems() {
  ...
  try {
    ...
    if (expiredEvents.length > 0 || expiredExtraClasses.length > 0) {
      console.log(
        `Auto-deleted ${expiredEvents.length} expired events and ${expiredExtraClasses.length} expired extra classes`
      );
    }
  } catch (error) {
...
export function setupModalCloseHandlers() {
  // Member modal
  document
    .getElementById("memberModalClose")
  ...
  // Events modal
  document
    .getElementById("eventsModalClose")
    ?.addEventListener("click", closeEventsModal);
  document
    .getElementById("eventsModalOverlay")
    ?.addEventListener("click", (e) => {
  ...
  // Routines modal
  document
    .getElementById("routinesModalClose")
    ?.addEventListener("click", closeRoutinesModal);
  document
    .getElementById("routinesModalOverlay")
    ?.addEventListener("click", (e) => {
  ...
  // Routine modal
  document
    .getElementById("routineModalClose")
    ?.addEventListener("click", closeRoutineModal);
  document
    .getElementById("routineModalOverlay")
    ?.addEventListener("click", (e) => {
